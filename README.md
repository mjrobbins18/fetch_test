...
This webservice is written in python, using django. This current iteration of this test runs on the local server. All of the code is located in the ```/points/urls.py``` file. In order to use this properly, it is important to create a virtual environment once the repository is cloned. In order to enter the virtual environment, type ```pipenv shell``` in the root directory. Django must also be installed once inside the virtual environment. It can be installed using ```pipenv install``` in the command line.
